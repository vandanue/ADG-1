...
indata=Line_001_geom_agc_wagc0.5_d2_fk_bpf15,20,70,80_decon_cdp.su
outdata=vpick2.data
...
dt=0.002		# Sample time interval (in seconds)
...
do
    ...
    #------------------------------------------------
    # Close all windows from the previous picking
    #------------------------------------------------
    echo "Closing previous ximage/xgraph windows..."
    pkill -f ximage 2>/dev/null
    pkill -f xgraph 2>/dev/null
    sleep 0.5
...
#------------------------------------------------
	...
	sunmo <panel.$picknow par=tmp2 |
		suximage title="CMP gather after NMO" xbox=10 ybox=10 \
				wbox=400 hbox=600 verbose=0 perc=90 &
