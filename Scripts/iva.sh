...
if [ ! -f Line_001_geom_cdp_d2_fk_bandpass_decon.su ]
...
indata=Line_001_geom_cdp_d2_fk_bandpass_decon.su
outdata=vpick.data

nt=1501			# ns in su header -> number of sample in trace
dt=0.002		# dt in second
...
#------------------------------------------------
    ...
    suxwigb <panel.$picknow xbox=422 ybox=10 \
             wbox=400 hbox=600 \
...
#------------------------------------------------
	...
	suxwigb <tmp1 xbox=834 ybox=10 wbox=400 hbox=600 \
...
#------------------------------------------------
	...
	suvelan < panel.$picknow nv=$nv dv=$dv fv=$fv |
		suxwigb xbox=10 ybox=10 wbox=400 hbox=600 \
...
#------------------------------------------------
	...
	sunmo <panel.$picknow par=tmp2 |
		suxwigb title="CMP gather after NMO" xbox=10 ybox=10 \
